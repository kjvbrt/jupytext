...
@pytest.mark.parametrize("subdir", ["md", ".md"])
def test_pair_file_in_non_existent_directory_path_873(
    tmp_path, python_notebook, subdir
):
    cm = jupytext.TextFileContentsManager()
    cm.root_dir = str(tmp_path)

    # Without this the creation of hidden subdirectories fail
    cm.allow_hidden = True

    nb = python_notebook
    nb.metadata["jupytext"] = {"formats": f"ipynb,{subdir}//md"}

    cm.save(notebook_model(nb), "test.ipynb")
